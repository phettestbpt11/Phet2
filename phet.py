...
def NOTIFIED_ACCEPT_GROUP_INVITATION(op):
    #print op
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + "สวัวดีคับ คุณ " + group.name)
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param3).displayName + " ลาก่อน ** \n(*´･ω･*)")
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + " ได้ออกจากลุ่ม * *\n(*´･ω･*)")
    except Exception as e:
        print e
            ...
            if msg.contentType == 0:
                ...
                if msg.text == "ลบเชิญ":
                    ...
                    if group.invitee is None:
                        ...
                        sendMessage(msg.to, str(len(group.invitee)) + " คน..\n ได้ทำการยกเลิก* *")
                ...
                if msg.text == "เวลา":
                    sendMessage(msg.to, "ขณะนี้เวลา" + datetime.datetime.today().strftime('ปี%Yเดือน%mวันที่%d %H:%M:%S') + "")
                if msg.text == "Gift":
                    sendMessage(msg.to, text="gift sent", contentMetadata=None, contentType=9)
                if msg.text == "อ่าน":
